...
export default function SettingsPage() {
  ...
  // Sync activeTab with URL search params on mount and URL change only
  useEffect(() => {
  ...
  const handleReorderUnits = async (reordered) => {
    setSettings(s => ({ ...s, units: reordered }));
    try {
      await reorderUnits(reordered.map(u => u.id));
    } catch {
      await fetchSettings();
    }
  };

  const handleReorderFaultCategories = async (reordered) => {
    setSettings(s => ({ ...s, faultCategories: reordered }));
    try {
      await reorderFaultCategories(reordered.map(c => c.id));
    } catch {
      await fetchSettings();
    }
  };

  const handleReorderStatuses = async (reordered) => {
    setSettings(s => ({ ...s, repairStatuses: reordered }));
    try {
  ...
  const handleReorderContractors = async (reordered) => {
    setSettings(s => ({ ...s, contractors: reordered }));
    try {
      await reorderContractorUnits(reordered.map(c => c.id));
    } catch {
      await fetchSettings();
    }
  };

  const handleReorderLocationsInUnit = async (unitId, reordered) => {
    setSettings(s => {
      const newLocations = s.locations.map(loc => {
        const reorderedItem = reordered.find(r => r.id === loc.id);
        return reorderedItem || loc;
      });
      return { ...s, locations: newLocations };
    });
    try {
      await reorderLocations(unitId, reordered.map(l => l.id));
    } catch {
      await fetchSettings();
    }
  };
  ...
  return (
    <div className="px-4 pb-4">
      ...
      {renderTabContent(activeTab, settings, handleAdd, handleUpdate, handleDelete,
        handleReorderUnits, handleReorderFaultCategories, handleReorderStatuses,
        handleReorderContractors, handleReorderLocationsInUnit)}
    </div>
  );
}
    ...
    case 'locations':
      return (
        <LocationSettingsSection
          ...
          onReorderInUnit={handleReorderLocationsInUnit}
        />
      );
    case 'faultCategories':
      return (
        <SettingsList
          ...
          onReorder={handleReorderFaultCategories}
          addTitle="新增故障類別"
        />
      );
    case 'repairStatuses':
    ...
    case 'contractors':
      return (
        <SettingsList
          ...
          onReorder={handleReorderContractors}
          addTitle="新增施工單位"
        />
      );
    default:
...
// ─── Location settings with per-unit grouping & drag-sort ────────────────────────
function LocationSettingsSection({ locations, units, onAdd, onUpdate, onDelete, onReorderInUnit }) {
  ...
  const handleAdd = async () => {
    ...
    const result = await onAdd({ name: newName, unit_id: parseInt(newUnitId) });
    if (result.success) {
      setNewName('');
      setNewUnitId('');
      setIsAdding(false);
  ...
  const startEdit = (item) => {
    ...
    setEditUnitId(String(item.unit_id || ''));
  };

  const cancelEdit = () => setEditingId(null);

  // Group locations by unit_id
  const grouped = {};
  locations.forEach(loc => {
    const uid = loc.unit_id;
    if (!grouped[uid]) grouped[uid] = [];
    grouped[uid].push(loc);
  });

  const renderItem = (item, unitName) => (
      ...
      {editingId === item.id ? (
        ...
        <>
          <div className="min-w-0">
            ...
            <span className="text-sm text-gray-500 ml-2">({unitName})</span>
          </div>
    ...
    <div className="space-y-3">
      ...
      {locations.length === 0 ? (
        ...
        <div className="space-y-6">
          {units?.map(unit => {
            const unitLocs = grouped[unit.id] || [];
            if (unitLocs.length === 0) return null;
            return (
              <div key={unit.id}>
                <h4 className="text-sm font-semibold text-gray-600 mb-2">{unit.name}</h4>
                <SortableList
                  items={unitLocs}
                  onReorder={(reordered) => onReorderInUnit(unit.id, reordered)}
                  renderItem={(item) => renderItem(item, unit.name)}
                />
              </div>
            );
          })}
          {/* Locations with unknown unit */}
          {Object.entries(grouped)
            .filter(([uid]) => !units?.find(u => u.id === parseInt(uid)))
            .map(([uid, locs]) => (
              <div key={uid}>
                <h4 className="text-sm font-semibold text-gray-400 mb-2">未分類</h4>
                <SortableList
                  items={locs}
                  onReorder={(reordered) => onReorderInUnit(parseInt(uid), reordered)}
...
function RepairStatusSettings({ items, onAdd, onUpdate, onDelete, onReorder }) {
  const [isAdding, setIsAdding] = useState(false);
  const [newName, setNewName] = useState('');
