...
async def list_units(
    ...
    Returns:
        List of units
    """
    units = db.query(Unit).filter(Unit.is_deleted == False).order_by(Unit.sort_order, Unit.id).all()
    return units
...
async def reorder_units(
    ...
    """
    Reorder units by updating sort_order for each id in the list.
    Admin only.
    """
    for idx, uid in enumerate(ordered_ids):
        unit = db.query(Unit).filter(Unit.id == uid, Unit.is_deleted == False).first()
        if unit:
            unit.sort_order = idx
    db.commit()
...
async def list_locations(
    ...
    locations = query.order_by(Location.sort_order, Location.id).all()
    return locations
...
async def create_location(
    ...
    # Check if location name exists for this unit (including soft-deleted)
...
async def reorder_locations(
    ...
    """
    Reorder locations within a unit by updating sort_order for each id in the list.
    Admin only.
    """
...
async def list_fault_categories(
    ...
    categories = query.order_by(FaultCategory.sort_order, FaultCategory.id).all()
    return categories
...
@router.put("/fault-categories/reorder")
async def reorder_fault_categories(
    ordered_ids: List[int],
    db: Session = Depends(get_db),
    current_user: Member = Depends(require_role(UserRole.ADMIN))
):
    """
    Reorder fault categories by updating sort_order for each id in the list.
    Admin only.
    """
    for idx, cid in enumerate(ordered_ids):
        cat = db.query(FaultCategory).filter(
            FaultCategory.id == cid,
            FaultCategory.is_deleted == False
        ).first()
        if cat:
            cat.sort_order = idx
    db.commit()
    return {"message": "Fault categories reordered"}
...
async def list_repair_statuses(
    ...
    """
    statuses = db.query(RepairStatus).filter(RepairStatus.is_deleted == False).order_by(RepairStatus.sort_order, RepairStatus.id).all()
    return statuses
...
async def create_repair_status(
    ...
    if existing:
        if existing.is_deleted:
            ...
            db.commit()
            db.refresh(existing)
            return {"id": existing.id, "name": existing.name, "color": existing.color or "#888888"}
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="此處理狀況名稱已存在"
        )
...
async def reorder_statuses(
    ...
    """
    Reorder repair statuses by updating sort_order for each id in the list.
    Admin only.
    """
    for idx, sid in enumerate(ordered_ids):
        st = db.query(RepairStatus).filter(
            RepairStatus.id == sid,
