...
async def get_all_settings(
    ...
    """
    units = db.query(Unit).filter(Unit.is_active == True, Unit.is_deleted == False).all()
    locations = db.query(Location).filter(Location.is_active == True, Location.is_deleted == False).all()
    fault_categories = db.query(FaultCategory).filter(
        FaultCategory.is_active == True,
        FaultCategory.is_deleted == False,
    ).all()
    repair_statuses = db.query(RepairStatus).filter(RepairStatus.is_active == True, RepairStatus.is_deleted == False, RepairStatus.name != "已存封").all()
